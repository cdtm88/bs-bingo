<script lang="ts">
  ...
  // myPlayerId never changes during a session — read once in onMount, not on every derived eval.
  ...
  onMount(() => {
    ...
    return () => {
      ...
      // Also disconnect here so the visibilitychange listener is always removed
      // on SvelteKit navigation teardown, even if onDestroy is not called (WR-02 fix).
      store?.disconnect();
    };
  });
  ...
  const canStart = $derived(wordCount >= MIN_WORDS_TO_START);
  ...
  function loadPack(pack: string) {
    store?.send({
      ...
      pack: pack as "corporate-classics" | "agile" | "it-jargon",
    });
  }
  ...
  $effect(() => {
    ...
    if (err?.code === "game_in_progress") {
      // Server rejected hello — game already underway, redirect home
      store?.clearError();
      goto("/");
    }
