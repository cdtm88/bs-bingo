...
test.describe("Phase 4: Win & Play-Again e2e", () => {
  test("both players see EndScreen within 1.5s of win", async ({ browser }) => {
    ...
    try {
      ...
      // Non-winner sees "{Host's name} called Bingo!" (SFW copy from copy.ts)
      await expect(b.getByText(/HostAlice called Bingo!/)).toBeVisible({ timeout: 1500 });

      // Non-winner does NOT see BINGO! wordmark
      await expect(b.getByText(/^BINGO!$/)).toHaveCount(0);

      // Winner does NOT see the "called Bingo!" heading variant (only the Display wordmark)
      await expect(a.getByText(/called Bingo!/)).toHaveCount(0);
  ...
  test("host starts new game, both players return to lobby with words retained", async ({
    ...
    try {
      ...
      // EndScreen UI gone — BINGO! wordmark and "called Bingo!" heading no longer visible
      await expect(a.getByText(/^BINGO!$/)).toHaveCount(0);
      await expect(b.getByText(/called Bingo!/)).toHaveCount(0);
    } finally {
      await ctxA.close();
      await ctxB.close();
    }
